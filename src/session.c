...
/* {{{ PYLIBSSH2_Session_set_timeout
 *  */
static char PYLIBSSH2_Session_set_timeout_doc[] = "\ 
set_timeout(timeout) -> void\n\
\n\
Set the timeout in milliseconds for how long a blocking the libssh2 function \n\
calls may wait until they consider the situation an error and return LIBSSH2_ERROR_TIMEOUT\n\
\n\
@param  timeout: milliseconds for blocking\n\
...
static PyObject *
PYLIBSSH2_Session_set_timeout(PYLIBSSH2_SESSION *self, PyObject *args)
{
    int timeout;
    if (!PyArg_ParseTuple(args, "l:set_timeout", &timeout)) {
